...
export default function App() {
  ...
  const registrarIngreso = async () => {
    // 1. Recopilamos toda la información de la sesión
    const payload = {
      estudiante: formData,
    ...
    try {
      ...
      if (response.ok) {
        ...
        setPasoActual(1); // Reiniciamos para el siguiente
      }
    } catch (error) {
      Alert.alert("Error de conexión", "No se pudo comunicar con el servidor central.");
    }
  };
  ...
  const renderPaso2 = () => (
    <View style={styles.stepContainer}>
      ...
      <View style={styles.grid}>
        <View style={styles.sensorCard}><Text style={styles.sensorTitle}>Tensiómetro</Text><Text style={styles.label}>PRESIÓN</Text><Text style={styles.metric}>{mediciones.presionSys}/{mediciones.presionDia} <Text style={styles.unit}>mmHg</Text></Text></View>
        <View style={styles.sensorCard}><Text style={styles.sensorTitle}>Banda de pecho</Text><View style={styles.row}><View style={{marginRight: 20}}><Text style={styles.label}>FC REPOSO</Text><Text style={styles.metric}>{mediciones.fcReposo} <Text style={styles.unit}>lpm</Text></Text></View><View><Text style={styles.label}>FC RECUP.</Text><Text style={styles.metric}>{mediciones.fcRecup} <Text style={styles.unit}>lpm</Text></Text></View></View></View>
        <View style={styles.sensorCard}><Text style={styles.sensorTitle}>Balanza</Text><View style={styles.row}><View style={{marginRight: 15}}><Text style={styles.label}>PESO</Text><Text style={styles.metric}>{mediciones.peso} <Text style={styles.unit}>kg</Text></Text></View><View style={{marginRight: 15}}><Text style={styles.label}>IMC</Text><Text style={styles.metric}>{mediciones.imc} <Text style={styles.unit}>kg/m²</Text></Text></View><View><Text style={styles.label}>GRASA</Text><Text style={styles.metric}>{mediciones.grasa} <Text style={styles.unit}>%</Text></Text></View></View></View>
        <View style={styles.sensorCard}><Text style={styles.sensorTitle}>Prueba esfuerzo</Text><Text style={styles.label}>VO₂ MÁX</Text><Text style={styles.metric}>{mediciones.vo2Max} <Text style={styles.unit}>ml/kg·min</Text></Text></View>
      </View>
      <TouchableOpacity style={styles.btnPrimary} onPress={simularMediciones}><Text style={styles.btnPrimaryText}>Volver a medir</Text></TouchableOpacity>
    </View>
  );
